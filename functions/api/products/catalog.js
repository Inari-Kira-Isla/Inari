...
export async function onRequestGet(context) {
  ...
  if (q) productUrl += `&or=(name.ilike.%25${encodeURIComponent(q)}%25,product_name_clean.ilike.%25${encodeURIComponent(q)}%25)`;
  ...
  const [prodResp, countResp, catResp] = await Promise.all([
    ...
    fetch(productUrl.replace(`&limit=${limit}&offset=${offset}`, "&select=id").replace("&order=category.asc,name.asc", ""), {
  ...
  if (catResp.ok) {
    ...
    categories = [...new Set(catData.map((r) => r.category).filter(Boolean))].sort();
  }
