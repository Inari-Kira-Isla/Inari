...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  if (typeof body.qty_counted !== 'number' || !Number.isFinite(body.qty_counted) || body.qty_counted < 0) {
    return jsonResponse({ error: 'invalid qty_counted' }, 400);
  }
  ...
  try {
    ...
    const rpcResponse = await fetch(`${SUPABASE_URL}/rest/v1/rpc/inventory_count`, {
      method: 'POST',
      headers: sbHeaders(key),
      body: JSON.stringify({
        p_tenant_id: TENANT_ID,
        p_lot_id: body.lot_id,
        p_qty_counted: body.qty_counted,
