...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  if (typeof body.qty_delta !== 'number' || !Number.isFinite(body.qty_delta)) {
    return jsonResponse({ error: 'missing qty_delta' }, 400);
  }
  ...
  try {
    ...
    const rpcResponse = await fetch(`${SUPABASE_URL}/rest/v1/rpc/inventory_adjust`, {
      method: 'POST',
      headers: sbHeaders(key),
      body: JSON.stringify({
        p_tenant_id: TENANT_ID,
        p_lot_id: body.lot_id,
        p_qty_delta: body.qty_delta,
        p_reason: body.reason,
        p_notes: body.notes ?? null,
        p_user: (locals.username as string | undefined) || 'admin',
